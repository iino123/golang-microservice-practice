...
// github_provider.goのioutil.ReadAll(response.Body)でエラーになるようなレスポンスをモックする
...
func TestCreateRepoInvalidErrorInterface(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "error when trying unmarshal GithubErrorResponse", err.Message)
}
...
func TestCreateRepoUnauthorized(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusUnauthorized,
			// 実際にgithubから返ってくるレスポンスを元にjson部分を作成する
...
func TestCreateRepoInvalidSuccessResponse(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "error when trying unmarshal CreateRepoResponse", err.Message)
}
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang-tutorial","full_name": "iino123-test/golang-tutorial"}`)),
		},
	})
	...
	assert.EqualValues(t, "iino123-test/golang-tutorial", response.FullName)
}
